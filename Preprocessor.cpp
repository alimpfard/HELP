...
Preprocessor::Preprocessor()
    : file_name(), lines(), source(), aliases(), magicChar('#'), kp_ext(false), rm_ws(false), rd_ws(false) {}

void Preprocessor::setFName(std::string& f) { file_name = f; }

void Preprocessor::setMagicChar(char mc) {magicChar = mc;}
...
void Preprocessor::handleSpecialMacro(const LineIterator& line)
{
    ...
    if(*line == "skip whitespace")
        ...
        rd_ws = !rd_ws;
    else if(*line == "keep extension")
        kp_ext = !kp_ext;
    else if(line->substr(0, 7) == "include")
        handleIncludeMacro(line->substr(line->find(' ') + 1), line);
    else if(line->substr(0,6) == "define")
        handleDefineMacro(line->substr(line->find(' ') + 1), line->substr(line->find(' ') + 1), line);
}

void Preprocessor::handleDefineMacro(const std::string& token, const std::string& defintion, const LineIterator& line){
    aliases.push_back(Alias(token,  defintion));
}
...
void Preprocessor::addMacros()
{
    auto it = lines.begin();
    while(it != lines.end()) {
        if(it->length() > 0 && it->at(0) == magicChar) {
            trim(*it);
            mergeLines(it);
...
void Preprocessor::applyHelpFile()
{
    ...
    std::cerr<<"Trying file "<<help_file<<" for directives."<<std::endl;
    std::ifstream hfs(help_file);
    if(!hfs.is_open()){
        std::cerr<<"Failed to open file. assuming inline directives."<<std::endl;
        return;}
    hfs.close();
